function feature_map = extract_features_multi(image, pos, scales, features, gparams, extract_info, i)
...
for sz_ind = 1:num_sizes
    ...
    for scale_ind = 1:num_scales
        if i == 1
            img_samples{sz_ind}(:,:,:,scale_ind) = sample_patch(image, pos + [ceil(img_sample_sz(1)*scales(scale_ind)/2), 0], img_sample_sz*scales(scale_ind), img_input_sz, gparams);
        elseif i == 2
            img_samples{sz_ind}(:,:,:,scale_ind) = sample_patch(image, pos - [ceil(img_sample_sz(1)*scales(scale_ind)/2), 0], img_sample_sz*scales(scale_ind), img_input_sz, gparams);
        elseif i == 3
            img_samples{sz_ind}(:,:,:,scale_ind) = sample_patch(image, pos + [0, ceil(img_sample_sz(2)*scales(scale_ind)/2)], img_sample_sz*scales(scale_ind), img_input_sz, gparams);
        elseif i == 4
            img_samples{sz_ind}(:,:,:,scale_ind) = sample_patch(image, pos - [0, ceil(img_sample_sz(2)*scales(scale_ind)/2)], img_sample_sz*scales(scale_ind), img_input_sz, gparams);
%             img_samples{sz_ind}(:,:,:,scale_ind) = sample_patch(image, pos - [0, 1000], img_sample_sz*scales(scale_ind), img_input_sz, gparams);
        end
    end
end
